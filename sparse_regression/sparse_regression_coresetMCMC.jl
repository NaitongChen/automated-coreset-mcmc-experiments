...
function main(args)
    ...
    model = CoresetMCMCSampler.SparseRegressionModel(length(data), D, data, datamat, 0.1, 0.1, 1, 10, 1/10, nothing)
    ...
    _, lp_evals, _, _, times, weights, metrics = CoresetMCMCSampler.sample!(kernel, model, cv, 0, rng, x -> compute_metric_cont(x, μp, dΣp), init_vals=init_vals)
...
function prepare_kernel(N::Int64, replicas::Int64, args::AbstractArray)
    ...
    if args[5] == "ADAM"
        ...
        if length(sizes) > 1
            kernel = CoresetMCMCSampler.CoresetMCMC(kernel = CoresetMCMCSampler.GibbsSR(), replicas = replicas, optimizer=CoresetMCMCSampler.ADAM(α = t -> sizes[1]/(t^sizes[2]), newton_correction=false), delay = 1, train_iter = parse(Int, args[2]), proj_n = proj_n, sum_to_N=false, non_neg=true, num_warmup=num_warmup)
        else
            kernel = CoresetMCMCSampler.CoresetMCMC(kernel = CoresetMCMCSampler.GibbsSR(), replicas = replicas, optimizer=CoresetMCMCSampler.ADAM(α = t -> sizes[1], newton_correction=false), delay = 1, train_iter = parse(Int, args[2]), proj_n = proj_n, sum_to_N=false, non_neg=true, num_warmup=num_warmup)
        end
    elseif args[5] == "DoG"
        sizes = parse.(Float64, split(args[4], "_"))
        kernel = CoresetMCMCSampler.CoresetMCMC(kernel = CoresetMCMCSampler.GibbsSR(), replicas = replicas, optimizer=CoresetMCMCSampler.DoG(r = sizes[1]), delay = 1, train_iter = parse(Int, args[2]), proj_n = proj_n, sum_to_N=false, non_neg=true, num_warmup=num_warmup)
    elseif args[5] == "DoWG"
        sizes = parse.(Float64, split(args[4], "_"))
        kernel = CoresetMCMCSampler.CoresetMCMC(kernel = CoresetMCMCSampler.GibbsSR(), replicas = replicas, optimizer=CoresetMCMCSampler.DoWG(rbar = sizes[1]), delay = 1, train_iter = parse(Int, args[2]), proj_n = proj_n, sum_to_N=false, non_neg=true, num_warmup=num_warmup)
    elseif args[5] == "ADAMDoGCoord"
        sizes = parse.(Float64, split(args[4], "_"))
        kernel = CoresetMCMCSampler.CoresetMCMC(kernel = CoresetMCMCSampler.GibbsSR(), replicas = replicas, optimizer=CoresetMCMCSampler.ADAMDoGCoord(r = sizes[1]), delay = 1, train_iter = parse(Int, args[2]), proj_n = proj_n, sum_to_N=false, non_neg=true, num_warmup=num_warmup)
    elseif args[5] == "ProdigyADAM_ca"
        sizes = parse.(Float64, split(args[4], "_"))
        kernel = CoresetMCMCSampler.CoresetMCMC(kernel = CoresetMCMCSampler.GibbsSR(), replicas = replicas, optimizer=CoresetMCMCSampler.prodigyADAM(ca=true, d=sizes[1]), delay = 1, train_iter = parse(Int, args[2]), proj_n = proj_n, sum_to_N=false, non_neg=true, num_warmup=num_warmup)
    end
    return kernel
end
