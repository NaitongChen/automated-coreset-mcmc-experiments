...
function main_ADAMDoGCoord()
    ...
    for c in [1:length(coreset_sizes);]
        ...
        for i in [1:size(labels_mix,1);]
            if i == 1
                plot!(metrics_mix[i], yscale=:log10, xscale=:log10, linecolor = ColorBrewer.palette("Oranges",7)[i+1], label=labels_no_mix[i], legend = :bottomleft, linwidth = 1)
            else
        ...
        savefig("plots/trace/linear_regression_coresetMCMC_metrics_mix_" * coreset_sizes[c] * "_" * alg[b] * ".png")
    end
end
...
function main_ADAMDoGCoord_combined()
    ...
    for c in [1:length(coreset_sizes);]
        ...
        for i in [1:size(labels_no_mix,1);]
            if i == 1
                plot!(metrics_no_mix[i], yscale=:log10, xscale=:log10, linecolor = ColorBrewer.palette("Blues",7)[i+1], label=labels_no_mix[i], legend = :bottomleft, linwidth = 1)
            else
        ...
        xlabel!("Iteration")
        ylabel!("Avg. Squared z-score")
        savefig("plots/trace/linear_regression_coresetMCMC_metrics_combined_" * coreset_sizes[c] * "_" * alg[b] * ".png")
    end
end