...
function main(args)
    ...
    model = CoresetMCMCSampler.PoissonRegressionModel(length(data), data, reduce(hcat, data)', d, 1, nothing)
    ...
    for i in [1:kernel.replicas;]
        θ0 = randn(rng_init, length(model.dataset[1])-1)
        push!(init_vals, θ0)
    end
    ...
    CSV.write("results/poisson_regression_coresetMCMC_metric_" * args[3] * "_" *args[4] * "_" * args[1] * "_" * args[5] * "_" * args[6] * ".csv",  Tables.table(metrics), writeheader=false)
    CSV.write("results/poisson_regression_coresetMCMC_lpeval_" * args[3] * "_" *args[4] * "_" * args[1] * "_" * args[5] * "_" * args[6] * ".csv",  Tables.table(lp_evals), writeheader=false)
    if args[1] == "1000"
        JLD2.save("results/poisson_regression_burnin" * args[3] * ".jld", "test_pass2_ratio", kernel.test_pass2_ratio,
                                                                            "lps", kernel.lps,
                                                                            "mmms", kernel.mmms,
